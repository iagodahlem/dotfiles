...
- `~/.config/zsh` is a directory link to `config/zsh`, so `ZDOTDIR` finds `.zprofile` (login shells only, OrbStack's shell init), `.zshrc`, `.p10k.zsh`, `.exports`, `.aliases`, `.functions` and `.bootstrap` there.
