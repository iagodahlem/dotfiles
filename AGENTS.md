...
- `scripts/utils/os.sh` provides shared `os_id()` detection (`DOTFILES_OS_ID` overrides it for tests); `scripts/utils/paths.sh` puts Homebrew and `~/.local/bin` on `PATH` for installer steps; `scripts/utils/lists.sh` provides `read_list_items`, the comment-stripping reader shared by the package lists and `config/links`, and `count_list_items`.
