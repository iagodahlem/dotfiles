...
- `scripts/install-shell.sh` sources `config/zsh/.zshenv`, then clones Oh My Zsh and tpm into the XDG data directory at their default branches, unpinned, and Powerlevel10k and the two zsh plugins into `$ZSH_CUSTOM` only where the OS has no package for them (`packages/` carries the formulae and the two plugins for pacman and apt; Powerlevel10k has no Debian, Ubuntu or Arch repository package). Rerunning it fast-forwards each clone (`git pull --ff-only`), so it is also the updater; Oh My Zsh's own updater stays off in `.zshrc`.
...
- `config/zsh/.zshrc` creates the state and cache directories, loads Oh My Zsh from `$ZSH` with no theme, then sources `config/zsh/.bootstrap`, then Powerlevel10k and zsh-autosuggestions, `.p10k.zsh`, and zsh-syntax-highlighting last. `find_zsh_addon` in `.bootstrap` picks each add-on's file: the Homebrew, Arch or Debian package path, else the clone in `$ZSH_CUSTOM`. `install-shell.sh` checks the same package paths (`packaged`), so change both together.
...
- Follow the phases in `PLAN.md`: installer, then host overlays and the host-level split (the packages and XDG layout phases are done).
