...
This installs packages (`brew bundle` on the core Brewfile, then on the one in the host overlay), links config into `~/.config` (plus `~/.zshenv`, the one file that stays in `$HOME`, see the README), sets up Oh My Zsh, its plugins and tpm, installs node, pnpm and bun through mise, installs the AI CLIs (claude, codex, gemini), syncs the LazyVim plugins, and applies macOS defaults (`TOOLS.md` lists them; key repeat applies after the next login). It runs the install hook of the host overlay last, when there is one. It opens with a banner (the host, the OS, the checkout, the overlay it found, the steps that run) and asks `Run these steps? [Y/n]` once, taking yes after 15 seconds; `--yes` skips the question. It ends with a summary of every step with its seconds, the warnings, and what is left for you: a reminder if `config/git/local` is still missing, to log out and back in for key repeat, and the Karabiner permissions below when this run installed it. If the private, mise, AI CLI, nvim or host step fails, the installer warns, carries on and lists the warning in the summary; rerun `scripts/install-private.sh`, `scripts/install-mise.sh`, `scripts/install-ai-clis.sh` or `scripts/install-nvim.sh` on their own once the cause is fixed, or the failed step with `./scripts/install.sh --only <step>`.
