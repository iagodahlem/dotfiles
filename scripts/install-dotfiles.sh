#!/usr/bin/env bash
set -euo pipefail

DOTS="${DOTS:-$HOME/.dotfiles}"
CONFIG_DIR="${DOTFILES_CONFIG_DIR:-$DOTS/config}"

safe_link() {
  local src="$1"
  local dst="$2"
  local ts

  if [ ! -e "$src" ]; then
    return 0
  fi

  if [ -L "$dst" ]; then
    rm -f "$dst"
  elif [ -e "$dst" ]; then
    ts="$(date +%Y%m%d%H%M%S)"
    mv "$dst" "${dst}.bak.${ts}"
  fi

  ln -s "$src" "$dst"
}

# git
safe_link "$CONFIG_DIR/git/.gitconfig" "$HOME/.gitconfig"
safe_link "$CONFIG_DIR/git/.gitignore_global" "$HOME/.gitignore_global"
safe_link "$CONFIG_DIR/git/.gitmessage" "$HOME/.gitmessage"

# zsh
safe_link "$CONFIG_DIR/zsh/.zshrc" "$HOME/.zshrc"
safe_link "$CONFIG_DIR/zsh/.p10k.zsh" "$HOME/.p10k.zsh"

# tmux
safe_link "$CONFIG_DIR/tmux/.tmux.conf" "$HOME/.tmux.conf"

# mise, nvm, atuin
safe_link "$CONFIG_DIR/mise/.tool-versions" "$HOME/.tool-versions"

if [ ! -e "$HOME/.gitconfig.override" ]; then
  echo "No ~/.gitconfig.override found: commits on this machine use the personal identity from config/git/.gitconfig."
  echo "On a work machine, copy config/git/.gitconfig.override.example to ~/.gitconfig.override and fill in the work email first."
fi
